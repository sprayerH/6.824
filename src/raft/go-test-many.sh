...
if ! go test -c -o ./test-tmp/tester; then
...
for i in $(seq "$((success+failed+1))" "$runs"); do
	...
	if [[ -z "$test" ]]; then
		./test-tmp/tester -test.v 2> "./test-tmp/test-${i}.err" > "./test-tmp/test-${i}.log" &
		pid=$!
	else
		./test-tmp/tester -test.run "$test" -test.v 2> "./test-tmp/test-${i}.err" > "./test-tmp/test-${i}.log" &
		pid=$!
	fi
